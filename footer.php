<div class="div-footer-largo">
	<footer id="footer1" class = "d-flex flex-column justify-content-end bg-dark w-100">
		<div class="d-flex justify-content-around h5">
			<a href="">
				<i class="fab fa-instagram text-secondary"> Instagram</i>
			</a>
			<a href="">
				<i class="fab fa-facebook-square text-secondary"> Facebook</i>
			</a>
			<a href="https://wa.me/5581993358655?text=Olá,%20tudo%20bem?%20" target="_blank">
				<i class="fab fa-whatsapp-square text-secondary"> whatsapp<br>81 993358655</i>
			</a>
			<p>
				<i class="fas fa-phone-square-alt text-secondary"> Telefone<br>81 992647007</i>
			</p>
			<p>					
				<i class="far fa-envelope text-secondary">
				e-mail<br>wcamposdealmeida@gmail.com</i>
			</p>
			<a href="sobre.php">
				<i class="fas fa-address-card text-secondary"> Sobre</i>
			</a>
		</div>
	</footer>
</div>

<div class="div-footer-fino">
	<footer id="footer2" class = "d-flex justify-content-around bg-dark w-100">
		<div class="d-flex flex-column justify-content-around">
			<a href="">
				<i class="fab fa-instagram text-secondary"> Instagram</i>
			</a>
			<a href="">
				<i class="fab fa-facebook-square text-secondary"> Facebook</i>
			</a>
			<a href="https://wa.me/5581993358655">
				<i class="fab fa-whatsapp-square text-secondary"> whatsapp<br>81 993358655</i>
			</a>
		</div>
		<div class="d-flex flex-column justify-content-around">
			<p>
				<i class="fas fa-phone-square-alt text-secondary"> Telefone<br>81 992647007</i>
			</p>
			<p>					
				<i class="far fa-envelope text-secondary">
				e-mail<br>wcamposdealmeida@gmail.com</i>
			</p>
			<a href="sobre.php">
				<i class="fas fa-address-card text-secondary"> Sobre</i>
			</a>
		</div>
	</footer>
</div>