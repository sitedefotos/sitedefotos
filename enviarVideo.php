<?php
	...
	for($i = 0; $i < count($arquivo['name']); $i++){	
		if ( isset( $arquivo[ 'name' ][$i] ) && $arquivo[ 'error' ][$i] == 0 ) {
		    ...
		    if ( strstr ( '.mp4', $extensao ) ) {

		 		$tamanho = $arquivo['size'][$i];

	 			include "conexao.php";

				//seleciona o nome mais alto (ultimo) dos videos
		        ...
		        $destino = 'imagens/ensaios/videos/'.$novoNome.$extensao;
		        // tenta mover o arquivo para o destino
		        if ( @move_uploaded_file ( $arquivo_tmp, $destino ) ) {
		        	$query = '
			 		insert into tb_fotos(nome, extensao, tamanho, estilo) values('.$novoNome.',"'.$extensao.'",'.$tamanho.',"video");';
					$conexao->exec($query);
